...
class Renderer:
    ...
    def render(self):
        ...
        for x in range(self.resolution):
            for y in range(self.resolution):
                forwards = self.player.get_forward(
                    (y - self.resolution / 2) + self.player.rotation.x,
                    (x - self.resolution / 2) + self.player.rotation.y
                )
                color, position, position_object = self.trace_ray(
    ...
    def distance_inaccurate(self, x1, x2, y1, y2, z1, z2):
        dx = x1 - x2
        dy = y1 - y2
        dz = z1 - z2
        return dx*dx + dy*dy + dz*dz

    def get_direction_vectors(self, vector1, vector2):
        vector = [vector2[0] - vector1[0], vector2[1] - vector1[1], vector2[2] - vector1[2]]
        vector_absolute = [abs(vector[0]), abs(vector[1]), abs(vector[2])]
        biggest_direction = max(vector_absolute)
        vector[0] /= biggest_direction
        vector[1] /= biggest_direction
        vector[2] /= biggest_direction
        return vector

    def trace_ray_to(self, x, y, z, xl, yl, zl, ray_resolution, position_object):
        difference_x = xl - x
        difference_y = yl - y
        difference_z = zl - z
        ...
        for i in range(ray_resolution):
            ...
            for object_checking in self.objects:
    ...
    def check_ray_intersects_sphere(self, ray_distance, ray_resolution, i, x, y, z, rx, ry, rz, object_radius, object_position, j, n, back_tracking):
        ...
        if self.distance(
            ...
            if not back_tracking:
                return self.check_ray_intersects_sphere(
    ...
    def check_ray_intersects_cube(self, ray_distance, ray_resolution, i, x, y, z, rx, ry, rz, object_size, object_position, j, n, back_tracking):
        increment = (ray_distance / ray_resolution) * (i + j)
        position = [
            x + rx * increment, 
            y + ry * increment, 
    ...
    def trace_ray(self, x, y, z, rx, ry, rz, ray_distance, ray_resolution):
        for i in range(ray_resolution):
            ...
            for object_checking in self.objects:
                ...
                if object_type == 1:
                    object_radius = object_checking.radius
                    distance_checked = self.distance_inaccurate(
                        object_position.x, position[0],
                        object_position.y, position[1],
                        object_position.z, position[2])
            ...
            if object_type == 1:
                ...
                if closest_object[1] <= object_radius*object_radius:
                        position_new = self.check_ray_intersects_sphere(
                ...
                object_size = object_checking.size
                position_new = self.check_ray_intersects_cube(
